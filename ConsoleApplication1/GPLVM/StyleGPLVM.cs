...
{
    public class StyleGPLVM : GP_LVM
    {
        ...
        private TopologicalPosition _topPos;
        ...
        public StyleGPLVM(int latentDimension = 2, ApproximationType aType = ApproximationType.ftc, Representation type = Representation.exponential, XInit initX = XInit.pca)
            : base(latentDimension, aType, BackConstType.ptc, initX)
        {
            _repType = type;
            _skeletonTree = null;
        }
        ...
        public override int LatentDimension
        {
            ...
            {
                if (_topPos == null)
                    return _q + _styles["Position"].SubStyles.Count;
                else
                    return _q + _topPos.Dimension;
            }
        }
        ...
        public override ILArray<double> LogParameter
        {
            get
            {
                using (ILScope.Enter())
                {
                    ...
                    if (_topPos != null)
                        param[ILMath.r(param.Length, param.Length + _topPos.NumParameter - 1)] = _topPos.LogParameter.C;
            ...
            set
            {
                using (ILScope.Enter())
                {
                    ...
                    {
                        startVal = endVal + 1;
                        endVal += _topPos.NumParameter;
                        _topPos.LogParameter = param[ILMath.r(startVal, endVal)];
                    }
        ...
        public TopologicalPosition TopPos
        {
            get { return _topPos; }
            set { _topPos = value; }
        }
        ...
        public override void Initialize()
        {
            ...
            foreach (IGPLVM node in Nodes)
            {
                ...
                if (_Y.IsEmpty)
                {
                    _Y = node.UpdateParameter().C;
                    _segments = node.Segments.C;
                }
                else
            ...
            _scale = ILMath.ones(1, _D);
            _bias = ILMath.mean(_Y);

            _M = ILMath.zeros(_Y.Size);
            ...
            {
                _indexes.Add(ILMath.localMember<double>());
                _indexes[_indexes.Count - 1].a = (ILMath.counter(_topPos.Dimension) + _q - 1);

                _X[ILMath.full, ILMath.r(ILMath.end + 1, ILMath.end + _topPos.Dimension)] = _topPos.Coordinates[_topPos.FactorIndex, ILMath.full];
                _q += _topPos.Dimension;
            }
            ...
            /*if (_topPos != null)
            {
                CompoundKern pos = new CompoundKern();
                pos.AddKern(new RBFKern());
                //pos.AddKern(new LinearKern());
                mult.AddKern(pos);
            }*/
            ...
            switch (_approxType)
            {
                ...
                case ApproximationType.dtc:
                    ...
                    if (_topPos != null)
                        _topPos.FactorIndexInducing = _topPos.FactorIndex[ind];
                    break;
        ...
        protected override ILRetArray<double> PreLogLikGradient()
        {
            ...
            //gX[ILMath.full, ILMath.r(0, _q + _styles["Position"].SubStyles.Count - 1)] += _latentgX; // top down gradient
            ...
            int cnt;
            ILArray<double> gTopPos = ILMath.empty();
            ILArray<double> gTopPosU = ILMath.empty();
            if (_topPos != null)
            {
                gTopPos = _topPos.PosGradient(gX[ILMath.full, _indexes[1]]);
                if (!gXu.IsEmpty)
                    gTopPosU = _topPos.PosInducingGradient(gXu[ILMath.full, _indexes[1]]);
                cnt = 2;
            }
            else
                cnt = 1;
            ...
            if (!gTopPos.IsEmpty)
                g[ILMath.r(g.Length, g.Length + gTopPos.Length - 1)] = gTopPos;
            if (!guStyle.IsEmpty)
                g[ILMath.r(g.Length, g.Length + gTopPosU.Length - 1)] = gTopPosU;
        ...
        public override ILRetArray<double> UpdateParameter()
        {
            ...
            _X = _X[ILMath.full, ILMath.r(0, _q - 1)];
            _Xu = _Xu[ILMath.full, ILMath.r(0, _q - 1)];

            _bias = ILMath.mean(_Y);
            _M = ILMath.zeros(_Y.S);
            for (int i = 0; i < _bias.Length; i++)
                _M[ILMath.full, i] = (_Y[ILMath.full, i] - _bias[i]) / _scale[i].C;
            ...
            if (_topPos != null)
            {
                _X[ILMath.full, ILMath.r(ILMath.end + 1, ILMath.end + _topPos.Dimension)] = _topPos.Coordinates[_topPos.FactorIndex, ILMath.full];
                if (!_Xu.IsEmpty)
                    _Xu[ILMath.full, ILMath.r(ILMath.end + 1, ILMath.end + _topPos.Dimension)] = _topPos.Coordinates[_topPos.FactorIndexInducing, ILMath.full];
            }
            ...
            //return retX
            return X[ILMath.full, ILMath.r(0, _q + _styles["Position"].SubStyles.Count - 1)];
        }
        ...
        public override void Read(ref XmlReader reader)
        {
            ...
            while (reader.NodeType != XmlNodeType.EndElement)
            {
                ...
                if (reader.Name == "TopologicalPosition")
                {
                    _topPos = new TopologicalPosition();
                    _topPos.Read(ref reader);
                }
        ...
        {
            writer.WriteStartElement("StyleGPLVM", null);
