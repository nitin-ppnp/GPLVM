...
{
    public class BackConstraintPTChalf : IBackconstraint
    {
        private int _N;
        private int _q;
        private int _D;
        ...
        /// </remarks>  
        public BackConstraintPTChalf(ILInArray<double> inInd = null)
        {
            _type = BackConstType.halfPtc;
            ...
            using (ILScope.Enter())
            {
                ...
                for (int i = 0; i < numSeg - 1; i++)
                {
                    ...
                    for (int j = (int)_segments[i]; j < _segments[i + 1]; j++)
                    {
                        ...
                        _phase[j] = (theta_0 + (j_1) * delta) / 2; // extracting the phase
                    }
                }
