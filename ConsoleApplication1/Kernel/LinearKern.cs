...
{
    [DataContract()]
    public class LinearKern : IKernel
    {
        [DataMember()]
        ...
        public LinearKern()
        {
            this._type = KernelType.KernelTypeLinear;
            ...
            using (ILScope.Enter(inX1, inX2))
            {
                ...
                switch (flag)
                {
                    case Flag.learning:
                        if (X1.S[0] == X2.S[0])
                        {
                            _K.a = _parameter[0] * ILMath.multiply(X1, X2.T);
                            k = _K;
                        }
                        else
                        {
                            _Kuf.a = _parameter[0] * ILMath.multiply(X1, X2.T);
                            k = _Kuf;
                        }
                        break;

                    case Flag.postlearning:
                        _Krec.a = _parameter[0] * ILMath.multiply(X1, X2.T);
                        k = _Krec;
                        break;

                    default:
                        k = _parameter[0] * ILMath.multiply(X1, X2.T);
                        break;
                }
        ...
        public ILRetArray<double> ComputeDiagonal(ILInArray<double> inX)
        {
            using (ILScope.Enter(inX))
            {
                ...
                return ILMath.sum(ILMath.multiplyElem(X, X), 1) * _parameter[0];
            }
        }
        ...
        public ILRetArray<double> LogLikGradientParamTensor(ILInArray<double> inX, ILInArray<double> indL_dK)
        {
            using (ILScope.Enter(inX, indL_dK))
            {
                ...
                ILArray<double> k = ComputeKernelMatrix(X, X, Flag.reconstruction);
                gParam[0] = ILMath.sum(ILMath.sum(ILMath.multiplyElem(dL_dK, k))) / _parameter[0];
            ...
            while (reader.NodeType != XmlNodeType.EndElement)
            {
                ...
                if (reader.Name == "parameter")
                {
                    ...
                    if (tokens.Length > 1)
                    {
                        if (tokens[1] == "*")
                        {
                            _parameter.a = ILMath.zeros(1, tokens.Length - 2);
