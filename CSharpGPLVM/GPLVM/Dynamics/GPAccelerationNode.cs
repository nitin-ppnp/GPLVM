...
{
    public class GPAccelerationNode : Node, IGPLVM
    {
        private int _numInducing = 200;   // number of inducing inputs

        private GPLVMType _type;          // the type of the kernel
        ...
        private List<ILArray<int>> _indexes;
        ...
        public GPAccelerationNode(ApproximationType aType = ApproximationType.ftc)
        {
            this._type = GPLVMType.dynamic;
        ...
        {
            get { return ILMath.zeros(1, _X.S[1]); }
        }
        ...
        public void Initialize()
        {
            using (ILScope.Enter())
            {
                ...
                {
                    node.Initialize();
                    _indexes.Add(ILMath.localMember<int>());

                    if (_X.IsEmpty)
                    {
                        _X.a = node.UpdateParameter().C;
                        _segments.a = node.Segments.C;
                        _indexes[_indexes.Count - 1].a = ILMath.counter<int>(0, 1, node.LatentDimension * 2) + curDimensions;
                    }
                    else
                    {
                        ytmp = _X.C;
                    ...
                    curDimensions += node.LatentDimension * 2;
                    ...
                    kernDynamicsParts.Add(kernCompound);
                }


                pgeKern.Kernels = kernDynamicsParts;
                pgeKern.Indexes = _indexes;
                _kern = pgeKern;
        ...
        protected virtual ILRetArray<double> PreLogLikGradient()
        {
            using (ILScope.Enter())
            {
                ...
                int cnt = 0, startVal = 0, dimCnt = 0;
                ILArray<double> gX = ILMath.empty();
                foreach (IGPLVM node in Nodes)
                {
                    gX = ILMath.zeros(_N, _indexes[cnt].Length);
                    ...
                    gX = Util.IndexAccum(gXin[ILMath.full, ILMath.r(dimCnt + node.LatentDimension, dimCnt + 2 * node.LatentDimension - 1)], _mapOrder3[ILMath.full, 1]);
                    gX = Util.ResizeAdd(gX, Util.IndexAccum(gXin[ILMath.full, ILMath.r(dimCnt, dimCnt + node.LatentDimension - 1)], _mapOrder3[ILMath.full, 0]));

                    // Setting gradients for Xout set of points ([2..end] for every segment)
                    gX = Util.ResizeAdd(gX, Util.IndexAccum(gXout[ILMath.full, ILMath.r(startVal, startVal + node.LatentDimension - 1)], _mapOrder3[ILMath.full, ILMath.end]));
                    ...
                    gX[_mapStart2[ILMath.full, 1], ILMath.full] -= _X[_mapStart2[ILMath.full, 1], ILMath.r(startVal, startVal + node.LatentDimension - 1)] 
                        - _X[_mapStart2[ILMath.full, 0], ILMath.r(startVal, startVal + node.LatentDimension - 1)];
                    // Gradient of [0] point of every segment is corrected to _X difference with next _X value
                    gX[_mapStart2[ILMath.full, 0], ILMath.full] += -_X[_mapStart2[ILMath.full, 0], ILMath.r(startVal, startVal + node.LatentDimension - 1)]
                                                                   + _X[_mapStart2[ILMath.full, 1], ILMath.r(startVal, startVal + node.LatentDimension - 1)]
                                                                   - _X[_mapStart2[ILMath.full, 0], ILMath.r(startVal, startVal + node.LatentDimension - 1)];

                    node.LatentGradient = gX;

                    g = Util.Concatenate<double>(g, node.LogLikGradient(), 1);

                    dimCnt += node.LatentDimension * 2;
                    startVal += node.LatentDimension;
                }
        ...
        public ILRetArray<double> PredictData(ILInArray<double> inTestInputs, ILOutArray<double> yVar = null)
        {
            ...
            using (ILScope.Enter())
            {
                ...
                switch (Mode)
                {
                    case LerningMode.prior:
                        ...
                        foreach (IGPLVM node in Nodes)
                        {
                            _X[ILMath.full, ILMath.r(startVal, startVal + node.LatentDimension - 1)] = node.UpdateParameter().C;

                            _Xin[ILMath.full, ILMath.r(dimCnt, dimCnt + node.LatentDimension - 1)] = _X[_mapOrder3[ILMath.full, 1], ILMath.r(startVal, startVal + node.LatentDimension - 1)];
                            _Xin[ILMath.full, ILMath.r(dimCnt + node.LatentDimension, dimCnt + 2 * node.LatentDimension - 1)] = _X[_mapOrder3[ILMath.full, 0], ILMath.r(startVal, startVal + node.LatentDimension - 1)];
