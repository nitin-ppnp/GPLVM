...
namespace GPLVM.Kernel
{
    ...
    public class RBFKern : IKernel
    {
        ...
        public RBFKern()
        {
            this._type = KernelType.KernelTypeRBF;

            double _variance = 1f;
            double _inverseScale = 1f;

            this._numParameter = 2;

            _parameter.a = new double[] {_inverseScale, _variance};
            ...
            using (ILScope.Enter(inX1, inX2))
            {
                ...
                switch(flag)
                {
                    case Flag.learning:
                        if (X1.S[0] == X2.S[0])
                        {
                            _n2.a = SqareDistance(X1, X2);
                            _K.a = _parameter[1] * ILMath.exp(-_n2 * 0.5f * _parameter[0]);
                            k = _K;
                        }
                        else
                        {
                            _n2uf.a = SqareDistance(X1, X2);
                            _Kuf.a = _parameter[1] * ILMath.exp(-_n2uf * 0.5f * _parameter[0]);
                            k = _Kuf;
                        }
                        break;

                    case Flag.postlearning:
                        _n2rec.a = SqareDistance(X1, X2);
                        _Krec.a = _parameter[1] * ILMath.exp(-_n2rec * 0.5f * _parameter[0]);
                        k = _Krec;
                        break;

                    default:
                        ILArray<double> n2 = SqareDistance(X1, X2);
                        k = _parameter[1] * ILMath.exp(-n2 * 0.5f * _parameter[0]);
                        break;
                }
        ...
        public ILRetArray<double> ComputeDiagonal(ILInArray<double> inX)
        {
            using (ILScope.Enter(inX))
            {
                ...
                return ILMath.repmat(_parameter[1], X.S[0], 1);
            }
        }
        ...
        public ILRetArray<double> LogLikGradientParamTensor(ILInArray<double> inX, ILInArray<double> indL_dK)
        {
            using (ILScope.Enter(inX, indL_dK))
            {
                ...
                ILArray<double> n = SqareDistance(X, X);
                ILArray<double> k = _parameter[1] * ILMath.exp(-n * 0.5f * _parameter[0]);

                ILArray<double> tmp = -0.5 * ILMath.multiplyElem(k, n);
                gParam[0] = ILMath.sum(ILMath.sum(ILMath.multiplyElem(dL_dK, tmp)));
                tmp = k / _parameter[1];
                gParam[1] = ILMath.sum(ILMath.sum(ILMath.multiplyElem(dL_dK, tmp)));
        ...
        public ILRetArray<double> DiagGradParam(ILInArray<double> inX, ILInArray<double> inCovDiag)
        {
            using (ILScope.Enter(inX, inCovDiag))
            {
                ...
                gParam[0] = 0;
                gParam[1] = ILMath.sum(CovDiag);

                return gParam; // because of the log space
            }
        }
