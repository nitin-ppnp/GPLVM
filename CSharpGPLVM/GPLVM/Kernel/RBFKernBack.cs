...
{
    [DataContract()]
    public class RBFKernBack : IKernel
    {
        [DataMember()]
        ...
        public RBFKernBack()
        {
            this._type = KernelType.KernelTypeRBFBack;

            double _inverseScale = 3e-4f;

            this._numParameter = 1;

            _parameter.a = new double[] {_inverseScale};
        }
                ...
                switch(flag)
                {
                    case Flag.learning:
                        if (X1.S[0] == X2.S[0])
                        {
                            ...
                            _K.a = ILMath.exp(-_n2 * 0.5f * _parameter[0]);
                            k = _K;
                        }
                        else
                        {
                            _n2uf.a = SqareDistance(X1, X2);
                            _Kuf.a = ILMath.exp(-_n2uf * 0.5f * _parameter[0]);
                            k = _Kuf;
                        }
                        break;
                    case Flag.postlearning:
                        _n2rec.a = SqareDistance(X1, X2);
                        _Krec.a = ILMath.exp(-_n2rec * 0.5f * _parameter[0]);
                        k = _Krec;
                        break;

                    default:
                        ILArray<double> n2 = SqareDistance(X1, X2);
                        k = ILMath.exp(-n2 * 0.5f * _parameter[0]);
                        break;
                }
