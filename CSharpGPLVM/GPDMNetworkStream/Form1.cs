    ...
    public partial class Form1 : Form
    {
        ...
        private void SyncroStream()
        {
            ...
            while (isStreaming)
            {
                ...
                //if (vicon.TIMESTAMP == 4803)
                //    avatar.StartFrame = true;
                ...
                if (computingisfinished && (isClicked || isClickedPlay) && avatarStream == null)
                {
                    //c3dStream = new Frame();
                    //c3dStream.NUM_ENTRIES = 4;
                    //c3dStream.ENTRIES = new FrameEntry[4];
                    //int cnt = 0;
                    //for (int i = 3; i < 7; i++)
                    //    c3dStream.ENTRIES[cnt++] = vicon.ENTRIES[i];

                    c3dStream = new Frame();
                    c3dStream.NUM_ENTRIES = 1;
                    c3dStream.ENTRIES = new FrameEntry[1];
                    c3dStream.ENTRIES[0] = vicon.ENTRIES[4];
