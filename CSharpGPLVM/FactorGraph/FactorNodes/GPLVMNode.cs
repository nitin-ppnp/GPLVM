...
namespace FactorGraph.FactorNodes
{
    ...
    public class GPLVMNode : FactorNodeWithKernel
    {
        ...
        public GPLVMNode(string sName, IKernel iKernel, ApproximationType aType = ApproximationType.ftc)
            : base(sName, iKernel)
        {
        ...
        protected ILArray<double> Inducing
        {
            get { return dcInducing.Values; }
            set { dcInducing.Values = value; }
        }
        ...
        public ILRetArray<double> PredictData(ILInArray<double> testInputs)
        {
            using (ILScope.Enter(testInputs))
            {
                ...
                {
                    case ApproximationType.ftc:
                        k = KernelObject.ComputeKernelMatrix(test, X, Flag.reconstruction);
                        break;
                    case ApproximationType.dtc:
                        k = KernelObject.ComputeKernelMatrix(test, Inducing, Flag.reconstruction);
                        break;
                    case ApproximationType.fitc:
                        k = KernelObject.ComputeKernelMatrix(test, Inducing, Flag.reconstruction);
                        break;
        ...
        private void UpdateKernelMatrix()
        {
            using (ILScope.Enter())
            {
                if (KernelObject != null)
                {
                    ...
                    {
                        case ApproximationType.ftc:
                            aK.a = KernelObject.ComputeKernelMatrix(X, X);
                        ...
                        case ApproximationType.dtc:
                            aK.a = KernelObject.ComputeKernelMatrix(Inducing, Inducing);
                            aKuf.a = KernelObject.ComputeKernelMatrix(Inducing, X);
                        ...
                        case ApproximationType.fitc:
                            ...
                            aKDiag.a = KernelObject.ComputeDiagonal(X);
        ...
        {
            return true; // Always optimiza X in GPLVM
            using (ILScope.Enter())
            {
            ...
            using (ILScope.Enter())
            {
                ...
                switch (pApproxType)
                {
                    case ApproximationType.ftc:
                        ...
                        if (OptimizingX())
                        {
                            gX = KernelObject.LogLikGradientX(X.C, dL_dK);
                        }
                    ...
                    case ApproximationType.dtc:
                        ...
                        dL_dX_dL_dXuf = KernelObject.LogLikGradientX(Inducing, dL_dK, X, dL_dKuf);
                    ...
                    case ApproximationType.fitc:
                        ...
                        ILArray<double> diagGX = KernelObject.DiagGradX(X);
                        for (int i = 0; i < nN; i++)
                            gX[i, ILMath.full] += diagGX[i, ILMath.full] * Kstar[i];

                        gKernelParams = KernelObject.LogLikGradientParam(dL_dKuf) + KernelObject.LogLikGradientParam(dL_dK);
                        gKernelParams += KernelObject.DiagGradParam(X, Kstar);
                ...
                // X ("latent" data) gradient (if needed)
                if (OptimizingX())
                {
                    dcX.SetGradient(gX);
                }
                ...
                if (OptimizingY())
                {
                    ILArray<double> gY = ILMath.zeros(X.Length, Scale.Length);
                    for (int i = 0; i < nQ; i++)
        ...
        public override void OnDeserializedMethod()
        {
            using (ILScope.Enter())
            {
                ...
                aInnerProducts = ILMath.localMember<double>();
                aBias = ILMath.localMember<double>();   
                aYCentered = ILMath.localMember<double>();
                ...
                aKDiag = ILMath.localMember<double>();   
                aDDiag = ILMath.localMember<double>();   
                aDInv = ILMath.localMember<double>();
                aDetDiff = ILMath.localMember<double>();
