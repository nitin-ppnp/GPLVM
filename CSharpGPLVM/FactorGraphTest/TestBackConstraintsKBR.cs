...
namespace FactorGraphTest
{
    ...
    {
        static ILRetArray<double> TestOriginalBackConstraintKBR(ILInArray<double> inData, ApproximationType aType)
        {
            using (ILScope.Enter(inData))
            {
                ILArray<double> data = ILMath.check<double>(inData);

                GP_LVM gplvm = new GP_LVM(nLatents, aType, BackConstType.kbr);
                gplvm.AddData(data);
        ...
        static ILRetArray<double> TestFactorGraphBackConstraintKBR(ILInArray<double> inData, ApproximationType aType)
        {
            using (ILScope.Enter(inData))
            {
                ILArray<double> data = ILMath.check<double>(inData);
                ...
                var fnBack = new KBRBackConstraintNode("Back constraints", kBack, dnX, dnY);
                ...
                fnBack.DataConnectorKernel.ConnectDataNode(dnKBack);
        ...
        static void TestBackConstraintKBR(ILInArray<double> inData, ApproximationType aType = ApproximationType.ftc, double fErrorTolarance = 1e-6)
        {
            using (ILScope.Enter(inData))
            {
                ILArray<double> data = ILMath.check<double>(inData);
                ...
                Console.WriteLine("\r\nOriginal GPDM with KBR BackConstraints:");
                ILArray<double> predictedOriginal = ILMath.empty();
                predictedOriginal.a = TestOriginalBackConstraintKBR(data, aType);
                Console.WriteLine("\r\nFactorGraph GPDM with BackConstraints:");
                ILArray<double> predictedFactorGraph = ILMath.empty();
                predictedFactorGraph.a = TestFactorGraphBackConstraintKBR(data, aType);
